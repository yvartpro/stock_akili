...
export default function NewProductModal({ isOpen, onClose, onProductCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-md shadow-xl overflow-hidden my-8">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-3.5 border-b border-slate-200 bg-slate-50">
          <div>
            <h3 className="font-bold text-base text-slate-900">Nouveau Produit au Stock Principal</h3>
            <p className="text-xs text-slate-500">Enregistrement d'une référence unitaire (Pochette ou Protection)</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`flex items-center justify-center gap-2 p-2.5 rounded-md border font-semibold text-xs transition-colors cursor-pointer ${
                  category === 'pochette'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-semibold text-slate-700">Marque de téléphone *</label>
                <button
                  type="button"
              ...
              {newBrandMode ? (
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-md"
                  >
                    OK
                  </button>
                </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-semibold text-slate-700">Modèle de téléphone *</label>
                <button
                  type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Couleur du produit *</label>
              <select
            ...
            {category === 'protection' ? (
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Type de protection anti-casse *</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Code SKU (Unique dans le stock)</label>
                <input
                  type="text"
                  value={sku}
          ...
          {/* Designation */}
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">Désignation commerciale complète</label>
            <input
              type="text"
              value={name}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Seuil Alerte Min</label>
              <input
                type="number"
                min="1"
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-semibold rounded-md shadow-xs transition-colors cursor-pointer flex items-center gap-2"
            >
